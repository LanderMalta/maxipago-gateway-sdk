...
exports.Gateway = (function () {
  ...
  function Gateway(merchantId, merchantKey, maxiPagoEnv) {
    ...
    this.addCustomer2 = async function (client, callback) {
      let clientXML = xmlBuilders.buildAddCustomerXML(
        client,
        MP_AUTH,
        XML_OPTIONS
      );
      await axios.post(MP_BASEURL + MP_POST_API, clientXML, MP_HEADERS)
        .then(function (response) {
          xml2js.parseString(response.data, XML_OPTIONS, function (err, obj) {
            if (err) return callback(err);
            return callback(obj.errorMessage, obj);
          });
        });
